    ...
    def update_status_label(self):
        ...
        for entry in self.load_all_meetings():
            if entry.get("date") == date:
                ...
                return
        self.status_label.configure(text="No meeting scheduled.")
        ...
        for meeting in data:
            if meeting.get("date") == date:
                if meeting["status"] == "scheduled":
                    ...
                    self.save_all_meetings(data)
                    messagebox.showinfo("Started", f"Meeting on {date} started.")
                    self.update_status_label()
                    return
                elif meeting["status"] == "started":
                    messagebox.showinfo("Info", "Meeting already started.")
                    return
                elif meeting["status"] == "completed":
                    messagebox.showinfo("Info", "Meeting already completed.")
                    return
        messagebox.showwarning("Not Found", f"No scheduled meeting on {date}.")
        ...
        for meeting in data:
            if meeting.get("date") == date:
                if meeting["status"] == "started":
                    ...
                    self.save_all_meetings(data)
                    messagebox.showinfo("Completed", f"Meeting on {date} marked as completed.")
                    self.update_status_label()
                    return
                elif meeting["status"] == "scheduled":
                    messagebox.showinfo("Info", "Meeting hasn't started yet.")
                    return
                elif meeting["status"] == "completed":
                    messagebox.showinfo("Info", "Meeting already completed.")
                    return
        messagebox.showwarning("Not Found", f"No meeting on {date}.")
